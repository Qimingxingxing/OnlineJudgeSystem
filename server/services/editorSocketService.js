...
module.exports = function(io) {
    ...
    io.on('connection', (socket) => {
        console.log("hi");
        // get sessionId from the handshake
        // we use problem.id as a sessionId
        const sessionId = socket.handshake.query['sessionId'];
        // put (socket id, sessionId) to the map
        socketIdToSessionId[socket.id] = sessionId;

        if (sessionId in collaborations) {
            // if there are some people working on this problem now
            collaborations[sessionId]['participants'].push(socket.id);
        } else {
            // no body is working on this problem right now, the person entered is the only one for now
            redisClient.get(sessionPath + sessionId, function(data) {
                if (data) {
                    // there were some people worked on this problem before.
                    // pulling the history data into memory from redis
